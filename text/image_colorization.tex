...
\subsubsection{Model autorski} \label{model autorski}
  ...
  Warstwa ósma ogranicza liczbę kanałów w sieci z 64 do 32 wyciągając z nich
...
\subsubsection{BatchNorm} \label{BatchNorm}
  ...
  ograniczanie maksymalnej wartości gradientu (ang. \textit{gradient clipping}) albo
...
\subsubsection{Modyfikacja rozdzielczości}
  ...
  zwana jest niezmiennością od translacji (ang. \textit{translation invariance}). Jest
...
\subsubsection{Wykorzystywany zbiór treningowy}
  ...
  tym idzie, stosunkowo niewielka liczba możliwych obiektów pojawiających się
  na zdjęciach powinna ułatwić sieci wyuczenie się właściwych barw dla
  identyfikowanych powierzchni. Z drugiej strony mała liczba klas może niekorzystnie
...
\subsubsection{Przetwarzanie wstępne danych} \label{Przetwarzanie wstępne danych}
  ...
  \item Zastosowanie rozmycia gaussowskiego (ang. \textit{Gaussian blur}) o różnej
  wielkości filtru Gaussowskiego.
  \end{itemize}

  Rozmycie gaussowskie, zwane także wygładzaniem gaussowskim (ang. \textit{Gaussian smoothing}),
...
\subsubsection{Augmentacja danych}
  ...
  \item Obcinanie skrajnych fragmentów obrazów (ang. \textit{crop}).
  ...
  \item Tworzenie niewielkich ubytków w obrazach w losowych miejscach (ang. \textit{coarse dropout}).
  \end{itemize}
...
\subsubsection{Funkcje kosztów} \label{Funkcje kosztów}
  ...
  \item MSELoss (ang. \textit{Mean Squared Error Loss}) - koszt oparty na błędzie średniokwadratowym,
  ...
  \item L1Loss zwany także MAELoss (ang. \textit{Mean Absolute Error Loss}) - koszt oparty na
...
\subsubsection{Funkcje aktywacji}
  ...
  PReLU (ang. \textit{Parametric ReLU}) oraz RReLU (ang. \textit{Randomized Leaky ReLU}) w
...
\subsubsection{Rezultaty} \label{Rezultaty}
 ...
 \end{figure}
  ...
  długość kroku treningowego $= 0.1$, współczynnik zaniku wag $= 1e-10$ oraz
  $eps=1e-08$.
  \newline
  Dla algorytmu AdaGrad zostały wybrane parametry: długość kroku treningowego $= 0.1$,
  współczynnik zaniku długości kroku treningowego $= 0.999$,
  współczynnik zaniku wag $= 1e-10$ oraz $eps=1e-10$.
  \newline
  Dla algorytmu SGD zostały wybrane parametry: długość kroku treningowego  $= 0.1$,
  pęd $= 0.9$, tłumienie $= 0$ oraz współczynnik zaniku wag $= 0$.
  Zmianę długości kroku treningowego dla SGD nadzorował planista o parametrach:
  wielkość kroku $= 1$ oraz gamma $= 0.999$.
  ...
  (ang. \textit{Residual Networks}) \cite{ResNet}
  ...
  (ang. \textit{upsampling}). Każdy człon konwolucyjny, oprócz dwóch ostatnich, składa się z pojedynczej
  ...
  zaletach, które znacznie zwiększa liczbę możliwych zastosowań sieci neuronowych.
