...
from keras.layers import Dense, Dropout, Flatten, Convolution2D, MaxPooling2D, UpSampling2D, BatchNormalization, \
    Reshape
...
def prepare_network(learning_rate, saving_period, models_dir, logs_dir):
    ...
    model.add(Convolution2D(4, (5, 5), activation=None, input_shape=(64, 64, 1)))
    model.add(BatchNormalization(axis=3))
    model.add(ReLU())

    model.add(Convolution2D(8, (5, 5), activation=None))
    model.add(BatchNormalization(axis=3))
    model.add(ReLU())
    ...
    model.add(Convolution2D(32, (5, 5), activation=None))
    model.add(BatchNormalization(axis=3))
    model.add(ReLU())

    model.add(Convolution2D(64, (5, 5), activation=None))
    model.add(BatchNormalization(axis=3))
    model.add(ReLU())

    model.add(Convolution2D(128, (5, 5), activation=None))
    model.add(BatchNormalization(axis=3))
    model.add(ReLU())
    ...
    model.add(Convolution2D(1, (5, 5), activation=None))
    model.add(ReLU())
    # model.add(BatchNormalization(axis=3))
